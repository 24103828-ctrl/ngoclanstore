...
function useProducts(filter?: (q: ReturnType<typeof supabase.from>) => unknown) {
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    let q: ReturnType<typeof supabase.from>["select"] extends never ? never : ReturnType<ReturnType<typeof supabase.from>["select"]>;
    const load = async () => {
    ...
    <UserLayout>
        ...
        <div className="container-px mx-auto max-w-7xl grid md:grid-cols-2 gap-8 items-center py-16 md:py-24">
          <div className="space-y-6 z-10">
            ...
            <h1 className="text-4xl md:text-6xl font-bold leading-[1.05]">
              Ngọc Lan Store
              <br />
              <span className="text-primary">Giày Thể Thao</span>
              <br />
              Chính Hãng
            </h1>
            <p className="text-lg text-secondary-foreground/70 max-w-md">
              Khám phá bộ sưu tập giày thể thao mới nhất từ các thương hiệu hàng đầu thế giới.
            </p>
            <div className="flex gap-3">
              <Link to="/products">
                <Button size="lg" className="rounded-full px-7 h-12 text-base font-semibold">
                  Mua Ngay <ArrowRight className="size-4 ml-2" />
                </Button>
              </Link>
              <Link to="/products">
          ...
          <div className="relative">
            ...
            <img src={heroShoe} alt="Hero" width={1600} height={1024} className="relative w-full h-auto rounded-2xl" />
          </div>
        </div>
      </section>
      ...
      <section className="border-y bg-background">
        <div className="container-px mx-auto max-w-7xl grid md:grid-cols-3 gap-8 py-10">
          {[
            { icon: Truck, t: "Miễn phí vận chuyển", d: "Đơn từ 500.000đ" },
            { icon: ShieldCheck, t: "Chính hãng 100%", d: "Cam kết hoàn tiền" },
            { icon: RefreshCw, t: "Đổi trả 30 ngày", d: "Dễ dàng & nhanh chóng" },
          ].map((f) => (
    ...
    <Section title={title}>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        {loading
          ...
          : <div className="col-span-full text-center text-muted-foreground py-10">Chưa có sản phẩm. Vui lòng chạy SQL setup.</div>}
      </div>
    </Section>
  );
}
