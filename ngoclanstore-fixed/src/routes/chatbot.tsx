...
function ChatPage() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("chatbot_history").select("*").eq("user_id", user.id).order("created_at").then(({ data }) => {
      setMsgs((data as ChatbotMessage[]) ?? []);
    });
  }, [user]);
  ...
  const send = async () => {
    ...
    setInput("");
    const userMsg: ChatbotMessage = { id: crypto.randomUUID(), user_id: user?.id ?? null, role: "user", message: text, created_at: new Date().toISOString() };
    setMsgs((m) => [...m, userMsg]);
    setLoading(true);
    try { await supabase.from("chatbot_history").insert({ user_id: user?.id ?? null, role: "user", message: text }); } catch {}
    try {
      const res = await fetch(WEBHOOK, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: text, userId: user?.id ?? "guest" }) });
      ...
      setMsgs((m) => [...m, aiMsg]);
      try { await supabase.from("chatbot_history").insert({ user_id: user?.id ?? null, role: "assistant", message: reply }); } catch {}
    } catch {
